...
public class HotelProfileFragment extends Fragment {
    ...
    private BasicServiceDialog currentBasicServiceDialog; // Para referenciar el dialog actual
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        // IMPORTANTE: Inicializar launchers PRIMERO
        setupPhotoLaunchers();
        ...
        photoPickerLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == getActivity().RESULT_OK && result.getData() != null) {
                        ...
                        if (imageUri != null) {
                            // Si hay un dialog de servicio básico abierto, enviarle la foto
                            if (currentBasicServiceDialog != null) {
                                currentBasicServiceDialog.addPhoto(imageUri);
                            } else {
                                // Es para fotos del hotel
    ...
    private void addHotelPhoto(Uri photoUri) {
        if (hotelPhotos.size() < 5) {
            hotelPhotos.add(photoUri);
            photosAdapter.notifyItemInserted(hotelPhotos.size() - 1);
            Toast.makeText(getContext(), "Foto del hotel agregada", Toast.LENGTH_SHORT).show();
        }
    }
